...
void Optimizer::getDoubles(double &T0, double &T1, double a, double b) {
    double tiny = 0.000001;
    T0 = 0, T1 = 0;
    if(b>tiny) {
        T1 = 1;
...
int Optimizer::getDoubleId(int x, int y, int N){
...
std::vector<double> Optimizer::checkConvexForT2Array(
const int triples[][4],
const long long &cnt,
bool save,
...
//return T[0..1]: convexity(in the form of loss/cost) pair in layout now
std::vector<double> Optimizer::checkConvexForT2withOld(
int triples[][4],
...
std::vector<double> Optimizer::checkCostForT2(
    ...
    if(global_N==N){
        ...
        triples_list = new long long[N*N*square_len*3][2];
