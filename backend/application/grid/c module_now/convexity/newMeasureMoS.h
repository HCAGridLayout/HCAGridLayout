...
// polygon form
std::vector<double> Optimizer::checkPolygonConvexByMoS(
    ...
    double alpha = 4)
{
    ...
    double C = std::max(area / area_hull, pow(perimeter_hull / perimeter, alpha));
    return {C, 1};
}


// grid form based on measure CHC and CHS
std::vector<double> Optimizer::checkConvexForMoS(
...
std::vector<double> Optimizer::checkConvexForMoSArray(
...
std::vector<double> Optimizer::checkCostForMoS(
