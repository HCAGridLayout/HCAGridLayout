...
void TSNE::constraint_compute_gaussian_perplexity(double *X, int N, double *constraint_X, int constraint_N, int D, int **_row_P, int **_col_P, double **_val_P, double perplexity, int K, int verbose, int forest_size, double accuracy)
{
	...
	*_val_P = (double *)calloc(constraint_N * K, sizeof(double));
	...
	if (*_row_P == NULL || *_col_P == NULL || *_val_P == NULL || offset == NULL || order == NULL)
	{
		printf("[t-SNE] Memory allocation failed!\n");
		exit(1);
	}
...
void TSNE::run_bhtsne(double *X, int N, int D, double *Y, double *P, int no_dim, double *constraint_X, double *constraint_Y,
					  ...
					  double *constraint_weight, int constraint_n_neighbors, int last_n)
{
#ifdef _OPENMP
	...
	double *val_P = new double[N * n_neighbors],
		   *constraint_val_P = new double[constraint_N * constraint_n_neighbors];
	binary_search_perplexity(distances_nn, neighbors_nn, N, n_neighbors, perplexity, val_P);
	constraint_binary_search_perplexity(constraint_distances_nn, constraint_neighbors_nn, constraint_N, constraint_n_neighbors, N, perplexity, constraint_val_P);
...
#pragma omp parallel for
	...
	for (int i = 0; i < N; i++)
	{
		for (int j = 0; j < n_neighbors; j++)
		{
			int pos = neighbors_nn[i * n_neighbors + j];
			P[i * N + pos] = val_P[i * n_neighbors + j];
		}
	}
...
#pragma omp parallel for
	...
	{
		constraint_indptr[i] = i * constraint_n_neighbors;
	}
	int *neighbors = new int[N * n_neighbors],
		*constraint_neighbors = new int[constraint_N * constraint_n_neighbors];
...
#pragma omp parallel for
#endif
	for (int i = 0; i < constraint_N * constraint_n_neighbors; i++)
	{
...
#pragma omp parallel for reduction(+ : sum)
	...
	for (int i = 0; i < constraint_N; i++)
	{
		for (int j = 0; j < constraint_n_neighbors; j++)
		{
			constraint_val_P[i * constraint_n_neighbors + j] /= sum;
			// printf("%f ", constraint_val_P[i * n_neighbors + j]);
			flag = flag || (abs_d(constraint_val_P[i * constraint_n_neighbors + j]) > 1.0);
		}
		// printf("\n");
	}
	// system("pause");
...
#pragma omp parallel for
	...
	for (int i = constraint_N * constraint_n_neighbors - 5; i < constraint_N * constraint_n_neighbors; ++i)
	{
		printf("%lf ", constraint_val_P[i]);
	}
	printf("\n");
	...
	for (iter = 0; iter < n_iter; iter++)
	{
		...
		*/
		bool need_eval_error = iter % n_iter_check == 0 && iter > 0 || iter == exploration_n_iter;
		// printf("point8\n");
...
#pragma omp parallel for
	...
	int *col_P;
	double *val_P;
	int *constraint_row_P;
	int *constraint_col_P;
	double *constraint_val_P;
...
extern "C"
{
	...
	__declspec(dllexport)
#endif
		extern void run_bhtsne(double *X, int N, int D, double *Y, double *P, int no_dim, double *constraint_X, double *constraint_Y,
							   int constraint_N, double alpha, double perplexity, double angle, int n_jobs,
							   ...
							   int constraint_n_neighbors, int last_n)
	{
		// srand(unsigned(time(0)));
		...
		if (constraint_N > 0)
		{
			tsne.run_bhtsne(X, N, D, Y, P, no_dim, constraint_X, constraint_Y, constraint_N, alpha, perplexity, angle, n_jobs,
							n_iter, random_state, forest_size, accuracy, early_exaggeration, learning_rate,
							skip_num_points, exploration_n_iter,
							n_neighbors, neighbors_nn, constraint_neighbors_nn, distances_nn, constraint_distances_nn, constraint_weight, constraint_n_neighbors, last_n);
		}
		else
		{
