...
std::vector<double> Optimizer::checkConvexForCArray(
...
int consider[]=nullptr, int consider_N=0, int consider_grid[]=nullptr) {
    ...
    checkEdgeArray4(E_grid, grid_asses, cluster_labels, N, num, square_len, maxLabel, consider, consider_N, consider_grid);
    ...
    // printf("check convex: ");
    // for(int i=0;i<maxLabel;i++)printf("%.2lf ", label_pairs[i][0]); printf("\n");
    // for(int i=0;i<maxLabel;i++)printf("%.2lf ", label_pairs[i][1]); printf("\n");
    // printf("check convex cost: %.2lf %.2lf\n", C0, C1);
...
std::vector<double> Optimizer::checkCostForC(
    ...
    std::vector<double> C_pair = checkConvexForCArray(grid_asses, cluster_labels, N, num, square_len, maxLabel, save, load, label_pairs, mainLabel1, mainLabel2, consider, consider_N, consider_grid);
    ...
    if((consider==nullptr)||(consider_N==0)) {
        ...
        for(int idx=0, gid=consider_grid[0];idx<consider_N;idx++, gid=consider_grid[idx]) {
            if(grid_asses[gid]<num) {
                element_asses[grid_asses[gid]] = gid;
                int i = grid_asses[gid];
                Similar_cost += Similar_cost_matrix[element_asses[i]*N+i];
                Compact_cost += Compact_cost_matrix[element_asses[i]*N+i];
                cost += Similar_cost_matrix[element_asses[i]*N+i] * (1-beta-alpha);
                cost += Compact_cost_matrix[element_asses[i]*N+i] * beta;
