...
void checkConnectDFS(
    ...
    int consider_e[]=nullptr) {
    int gid = x*square_len+y;
    int id = grid_asses[gid];
    if((consider_e!=nullptr)&&(!consider_e[id]))return;
...
double checkConnectForAll(
    ...
    int consider_e[]=nullptr, 
    int consider_N=0, int consider_grid[]=nullptr) {

    int *a = new int[N];
    int *b = new int[maxLabel];

    if((consider_e==nullptr)||(consider_N==0)) {
        for(int i=0;i<N;i++)a[i] = 0;
    }else {
        for(int i=0;i<consider_N;i++) a[i] = 0;
    }
    ...
    if((consider_e==nullptr)||(consider_N==0)) {
        ...
        for(int idx=0, i=consider_grid[0];idx<consider_N;idx++, i=consider_grid[idx]) checked[i] = 0;
    }
    ...
    if(if_disconn!=nullptr){
        if((consider_e==nullptr)||(consider_N==0)) {
            ...
            for(int idx=0, i=consider_grid[0];idx<consider_N;idx++, i=consider_grid[idx]) if_disconn[i] = false;
        }
    }
    ...
    if((consider_e==nullptr)||(consider_N==0)) {
        ...
        if(grid_asses[gid]<num){
            int id = grid_asses[gid];
            if((consider_e!=nullptr)&&(!consider_e[id]))continue;
            int lb = cluster_labels[id];
            if(checked[gid]>0)continue;
            int x = gid/square_len;
            int y = gid%square_len;
            checkConnectDFS(grid_asses, cluster_labels, checked, check_cnt++, x, y, gid, lb, num, square_len, connect, consider_e);
        }
    }else {
        for(int idx=0, gid=consider_grid[0];idx<consider_N;idx++, gid=consider_grid[idx])
        if(grid_asses[gid]<num){
            int id = grid_asses[gid];
            if((consider_e!=nullptr)&&(!consider_e[id]))continue;
            int lb = cluster_labels[id];
    ...
    if((consider_e==nullptr)||(consider_N==0)) {
        ...
        for(int idx=0, gid=consider_grid[0];idx<consider_N;idx++, gid=consider_grid[idx]) {    // every positon
            double x = 1.0*(gid/square_len)/square_len;
            double y = 1.0*(gid%square_len)/square_len;
            int id = grid_asses[gid];
            if((consider_e!=nullptr)&&(!consider_e[id]))continue;
...
bool checkDisconnectChange(
    ...
    if((consider==nullptr)||(consider_N==0)) {
        for(int gid=0;gid<N;gid++){
            if((consider!=nullptr)&&(!consider[gid]))continue;
            if(if_disconn[gid]!=if_disconn2[gid]){
                return true;
            }
        }
    }else {
...
double checkMainConnectForAll(
    ...
    int *c = new int[maxLabel];
    for(int i=0;i<maxLabel;i++)c[i] = -1;
    ...
    if(grid_asses[gid]<num){
        ...
        if((consider_e!=nullptr)&&(!consider_e[id]))continue;
        int lb = cluster_labels[id];
        if((c[lb]<0)||(a[checked[gid]-1]>a[c[lb]]))c[lb] = checked[gid]-1;
    }
...
std::vector<bool> getConnectBorder(
    ...
    int *consider_e = new int[num];
    for(int i=0;i<num;i++)consider_e[i] = _consider_e[i];
    int *checked = new int[N];
    bool *if_disconn = new bool[N];

    double cost = checkConnectForAll(grid_asses, cluster_labels, checked, N, num, square_len, maxLabel, 4, if_disconn, consider_e);
    std::vector<bool> ret(num, false);

    for(int gid1=0;gid1<N;gid1++)
    if((!if_disconn[gid1])&&(grid_asses[gid1]<num)) {
        int id1 = grid_asses[gid1];
        int x1 = gid1/square_len;
        int y1 = gid1%square_len;
        int lb1 = maxLabel;
        if(!consider_e[id1])continue;
        ...
        for(int xx=-1;xx<=1;xx++) {    // grids[x1+xx][y1+yy]
            ...
            for(int yy=-1;yy<=1;yy++) {
                ...
                int lb2 = maxLabel;
                if(id2<num)lb2 = cluster_labels[id2];    // label(cluster) of the element
                if(lb1!=lb2) ret[id1] = true;
            }
        }
    }
...
std::vector<std::vector<std::vector<std::vector<double>>>> getConnectShape(
    const std::vector<int> &_grid_asses,
    const std::vector<int> &_cluster_labels,
    const std::vector<int> &_consider_e) {
    int N = _grid_asses.size();
    ...
    std::vector<std::vector<std::vector<std::vector<double>>>> ret;
    ...
    if((!if_disconn[gid1])&&(grid_asses[gid1]<num)) {
        ...
        int x1 = gid1/square_len;
        int y1 = gid1%square_len;
        double x = 1.0*x1/square_len;
        double y = 1.0*y1/square_len;
        int lb1 = maxLabel;
        if(!consider_e[id1])continue;
        if(id1<num)lb1 = cluster_labels[id1];
        else continue;
        ...
        for(int xx=-1;xx<=1;xx++) {    // grids[x1+xx][y1+yy]
            ...
            for(int yy=-1;yy<=1;yy++) {
                ...
                int y2 = y1+yy;
                int flag = 0;
                if((x2<0)||(x2>=square_len)||(y2<0)||(y2>=square_len))flag = 1;
                else {
                    int gid2 = bias2+y2;
                    int id2 = grid_asses[gid2];
                    int lb2 = maxLabel;
                    if(id2<num)lb2 = cluster_labels[id2];    // label(cluster) of the element
                    if((lb1!=lb2)||(!consider_e[id2]))flag = 1;
                }
                if(flag>0) {
                    std::vector<std::vector<double>> b;
                    std::vector<double> c(2);
                    if(xx!=0){
                        c[0] = x+(1.0+xx)/2/square_len;
                        c[1] = y;
                        b.push_back(c);
                        c[0] = x+(1.0+xx)/2/square_len;
                        c[1] = y+1.0/square_len;
                        b.push_back(c);
                    } else {
                        c[0] = x;
                        c[1] = y+(1.0+yy)/2/square_len;
                        b.push_back(c);
                        c[0] = x+1.0/square_len;
                        c[1] = y+(1.0+yy)/2/square_len;
                        b.push_back(c);
                    }
                    ret[lb1].push_back(b);
...
std::vector<bool> getMainConnectGrids(
    const std::vector<int> &_grid_asses,
    const std::vector<int> &_cluster_labels,
    const std::vector<int> &_consider) {
    int N = _grid_asses.size();
    int num = _cluster_labels.size();
    int square_len = ceil(sqrt(N));
    int maxLabel = 0;
    ...
    for(int i=0;i<N;i++){
        consider[i] = _consider[i];
        consider_e[grid_asses[i]] = consider[i];
    }
    int *checked = new int[N];
    bool *if_disconn = new bool[N];

    double cost = checkMainConnectForAll(grid_asses, cluster_labels, checked, N, num, square_len, maxLabel, 4, if_disconn, consider_e);
...
void getOtherCostMatrixArrayToArray(
int grid_asses[],
int cluster_labels[],
double ret_a[],
const int &N, const int &num, const int &square_len, const int &maxLabel,
...
int consider[]=nullptr) {
    ...
    for(int i=0;i<N*(maxLabel+1);i++)cost_matrix[i] = 0;
//    memset(ret_a, 0, N*N*sizeof(double));

    for(int lb=0;lb<maxLabel;lb++){
        if(!is_other_label[lb])continue;
        double cluster_x = 0, cluster_y = 0;
        int tot = 0;
        for(int gid1=0;gid1<N;gid1++){    // grids
            int id1 = grid_asses[gid1];

            int lb0 = maxLabel;
            if(id1<num)lb0 = cluster_labels[id1];    // label(cluster) of the element
            if(lb!=lb0)continue;
        ...
        for(int gid1=0;gid1<N;gid1++){    // grids
            double x = 1.0*(gid1/square_len)/square_len;
            double y = 1.0*(gid1%square_len)/square_len;
            cost_matrix[gid1*(maxLabel+1)+lb] = getDist(x, y, cluster_x, cluster_y);
        }
        ...
        for(int gid1=0;gid1<N;gid1++){    // grids
            ...
            for(int xx=-1;xx<=1;xx++) {    // grids[x1+xx][y1+yy]
                ...
                for(int yy=-1;yy<=1;yy++) {
                    ...
                    if((y2<0)||(y2>=square_len))continue;
                    int gid2 = bias2+y2;
                    cost_matrix[gid2*(maxLabel+1)+lb] = 0;
                }
            }
        }     
    }
    ...
    for(int gid1=0;gid1<N;gid1++){    // grids
        ...
        if(!is_other_label[lb])continue;
        for(int gid2=0;gid2<N;gid2++) {    // position of elements in layout now
...
int consider[]=nullptr) {
    ...
    for(int gid=0;gid<N;gid++) {    // grids
        if((consider!=nullptr)&&(consider[gid]==0))continue;
        int bias = gid*N;
...
//count the edges of grid "gid" with other clusters
int checkEdgeSingle(
const int &gid,
const int grid_asses[],
const int cluster_labels[],
const int &N, const int &num, const int &square_len, const int &maxLabel) {
    ...
    int id = grid_asses[gid];
    int lb0 = maxLabel;
    if(id<num)lb0 = cluster_labels[id];
...
const int cluster_labels[],
const int &N, const int &num, const int &square_len, const int &maxLabel,
int consider[]=nullptr, int consider_N=0, int consider_grid[]=nullptr) {
    ...
    if((consider==nullptr)||(consider_N==0)) {
        ...
        for(int idx=0, i=consider_grid[0];idx<consider_N;idx++, i=consider_grid[idx]) {
            E_grid[i] = 0;
        }
        // #pragma omp parallel for num_threads(THREADS_NUM)
        for(int idx=0, gid=consider_grid[0];idx<consider_N;idx++, gid=consider_grid[idx]) {
