...
class IncrementalTSNE:
    ...
    def __init__(self, n_components=2, perplexity=30.0, early_exaggeration=12.0,
        ...
        self.ffi = cffi.FFI()
        self.ffi.cdef(
            """void run_bhtsne(double* X, int N, int D, double* Y, double* P, int no_dim, double* constraint_X, double* constraint_Y,
    ...
    def _fit(self, X, skip_num_points=0, constraint_X=None, constraint_Y=None, alpha=0, corrected_id=None, corrected_weight=None, init_id=None, constraint_labels=None, labels=None, label_alpha=1, constraint_weight=None, prev_n=0):
        ...
        if self.method == "exact":
            ...
            if constraint_N > 0:
                all_label = np.unique(np.concatenate((labels, constraint_labels)))
                if all_label[0] == -1:
                    all_label = all_label[1:]
            else:
                all_label = np.unique(labels)
            n_labels = len(all_label) # n_labels = 0
            means = []
            means_embed = []
            distribution = {}
            if n_labels > 0:
            ...
            distances = np.array(distances, copy=True).reshape(-1) ** 2
            ...
            if constraint_N > 0:
                ...
                for i in range(X.shape[0]):
                    ...
                    constraint_indices.append(ret[0][0:])
                    if labels[i] == -1:
                        constraint_distances.append(ret[1][0:])
                    else:
                        dist = []
                ...
                constraint_n_neighbors = np.array(constraint_distances).shape[1]
                constraint_indices = np.array(constraint_indices, copy=True).reshape(-1)
                constraint_distances = np.array(constraint_distances, copy=True).reshape(-1) ** 2
            ...
            X = X.reshape(-1)
            Y = Y.reshape(-1)
            P = np.zeros((N + n_labels, N + n_labels), dtype=np.float64).reshape(-1)
            cffi_X = self.ffi.cast('double*', X.ctypes.data)
            cffi_Y = self.ffi.cast('double*', Y.ctypes.data)
            cffi_P = self.ffi.cast('double*', P.ctypes.data)
            ...
            t = FuncThread(self.C.run_bhtsne, cffi_X, N + n_labels, D, cffi_Y, cffi_P, self.n_components,
            ...
            ret_P = P.reshape((N+n_labels, N+n_labels))[:N, :N]
            # np.save("P.npy", ret_P)
            return ret[:N]
...
if __name__ == "__main__":
    ...
    X_embedded = tsner.fit_transform(X, labels=[0, 0, 1, 1], label_alpha=0.9)
    print(X_embedded)