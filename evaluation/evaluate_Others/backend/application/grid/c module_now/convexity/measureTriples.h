...
int Optimizer::getHash(int N, int consider[]=nullptr){
    int hash = 0, t = 1;
    if(consider == nullptr)return -1;
    for(int i=0;i<N;i++){
        hash += consider[i]*t;
        t = t*3;
    }
...
void Optimizer::checkTriplesOfLine2(
int triples[][4],
    ...
    while(nx<targetX){
        ...
        while(ty<tp){
            ...
            if((gid!=gid1)&&(gid!=gid2)){
                if((consider!=nullptr)&&(!consider[gid]||!consider[gid1]||!consider[gid2])){
                }else {
                    triples[cnt][0] = gid1;
                    triples[cnt][1] = gid;
                    triples[cnt][2] = gid2;
...
void Optimizer::checkTriplesOfLine(
int triples[][4],
long long triples_head[],
long long triples_list[][2],
long long &cnt,
...
const int &gid1, const int &gid2,
int consider[]=nullptr){
    if((consider!=nullptr)&&(!consider[gid1]||!consider[gid2]))return;
    ...
    if(startX==endX) {
        int bt = std::min(startY, endY);
        int tp = std::max(startY, endY);
        int x = startX;
        int y = bt + 1;
        while(y<tp){
            int gid = x*square_len+y;
        ...
        int bt = std::min(startX, endX);
        int tp = std::max(startX, endX);
        int y = startY;
        int x = bt + 1;
        while(x<tp){
            int gid = x*square_len+y;
        ...
        cnt, square_len, gid1, gid2, oriX, oriY, targetX, startX, dx, dy, consider);
    }
}

//check triples and calculate convexity in an oblique line segment
void Optimizer::getConvexForTOfLine2(
double &T0,
double &T1,
...
//check triples and calculate convexity in an line segment
void Optimizer::getConvexForTOfLine(
double &T0,
double &T1,
const int grid_asses[],
const int cluster_labels[],
...
int consider[]=nullptr){
    ...
    if(startX==endX) {
        ...
        int oriY = (startY*2+1)*dx;
        int targetX = endX*2+1;
        getConvexForTOfLine2(T0, T1, grid_asses, cluster_labels, 
        N, num, square_len, gid1, gid2, oriX, oriY, targetX, startX, dx, dy, consider);
    }
}

//check and save triples
long long Optimizer::getTriples(
int triples[][4],
long long triples_head[],
long long triples_list[][2],
const int grid_asses[],
...
int consider[]=nullptr){
    ...
    for(int gid2=gid1+1;gid2<N;gid2++)checkTriplesOfLine(triples, triples_head, triples_list, cnt, square_len, gid1, gid2, consider);

    return cnt;
}

//get triples and checkconvexity
std::vector<double> Optimizer::getTriplesAndConvexForT(
const int grid_asses[],
const int cluster_labels[],
const int &N, const int &num, const int &square_len, const int &maxLabel,
int consider[]=nullptr){
    ...
    double T1=0;
    for(int gid1=0;gid1<N-1;gid1++)
    for(int gid2=gid1+1;gid2<N;gid2++)getConvexForTOfLine(T0, T1, grid_asses, cluster_labels, N, num, square_len, gid1, gid2, consider);
...
std::vector<int> Optimizer::checkTriplesByDict(
...
void Optimizer::updateTripleDict(
int innerDict[], int outerDict[],
const int triples[][4],
...
const int &N, const int &num, const int &square_len, const int &maxLabel,
const int gid) {
    for(long long ii=triples_head[gid];ii>=0;ii=triples_list[ii][1])
    {
        long long i = triples_list[ii][0];
...
void Optimizer::updateTripleDictwithOld(
...
const int gid) {
    ...
    for(long long ii=change_triples_head[gid];ii>=0;ii=change_triples_list[ii][1])
    {
        // printf("ii %d\n", ii);
        long long i = change_triples_list[ii][0];
        // printf("update %d %d\n", gid, i);
...
std::vector<double> Optimizer::checkConvexForTArray(
const int triples[][4],
const long long &cnt,
const int grid_asses[],
const int cluster_labels[],
const int &N, const int &num, const int &square_len, const int &maxLabel) {
    ...
    if (triples==nullptr){
        return getTriplesAndConvexForT(grid_asses, cluster_labels, N, num, square_len, maxLabel);
    }

    #pragma omp parallel for reduction(+:T0, T1) num_threads(THREADS_NUM)
    for(int starti=0;starti<N;starti++){
        for(long long i=starti;i<cnt;i+=N){
            int gid1 = triples[i][0];
            int gid0 = triples[i][1];
            int gid2 = triples[i][2];
            int times = triples[i][3];
...
std::vector<double> Optimizer::checkConvexForT(
...
const std::vector<int> &_cluster_labels) {
    ...
    long long cnt;
    int hash = -1;
//    printf("global and now %d %d %d %d\n", global_N, global_hash, N, hash);
    if((global_N==N)&&(global_hash==hash)){    //use saved triples link list
...
//            printf("delete triples %d %d\n", global_N, global_hash);
        ...
        long long max_cnt = 1;
        max_cnt = max_cnt*N*N*square_len;
        // if (max_cnt<=50*50*50*50*50) {
        if (true) {
            triples = new int[max_cnt][4];
            triples_head = new long long[N];
            triples_list = new long long[max_cnt*3][2];
            ...
            global_hash = hash;
            global_N = N;
//        printf("refresh triples\n");
        }else {
            triples = nullptr;
            triples_head = nullptr;
            triples_list = nullptr;
            cnt = 0;
        }
    }
...
//return T[0..1]: convexity(in the form of loss/cost) pair in layout now
std::vector<double> Optimizer::checkConvexForTwithOld(
int triples[][4],
long long triples_head[],
long long triples_list[][2],
double old_T_pair[],
...
long long Optimizer::getChangeTriplesCnt(
const long long cnt,
int triples[][4],
long long triples_head[],
long long triples_list[][2],
...
const int &N, const int &num, const int &square_len, const int &maxLabel) {
    ...
    for(int gid=0;gid<N;gid++) {
        if(changed[gid]==1){
            for(long long ii=triples_head[gid];ii>=0;ii=triples_list[ii][1])change_cnt += 1;
        }
    }
...
long long Optimizer::getChangeTriples(
...
void Optimizer::getCostMatrixForTArrayToArray(
int grid_asses[],
int cluster_labels[],
...
int consider[]=nullptr) {
    ...
    long long *change_triples_head;
    long long (*change_triples_list)[2];
    long long change_cnt;

    int hash = getHash(N, consider);
    int consider_N = N;
    if (consider!=nullptr) {
        consider_N = 0;
        for(int gid=0;gid<N;gid++)if(consider[gid]){
            consider_N += 1;
        }
    }
//    printf("global and now %d %d %d %d\n", global_N, global_hash, N, hash);
...
//            printf("delete triples %d %d\n", global_N, global_hash);
        ...
        long long max_cnt = 1;
        max_cnt = max_cnt*consider_N*consider_N*square_len;
        triples = new int[max_cnt][4];
        triples_head = new long long[N];
        triples_list = new long long[max_cnt*3][2];

        cnt = getTriples(triples, triples_head, triples_list, grid_asses, cluster_labels, N, num, square_len, maxLabel, consider);
        global_triples = triples;
        global_triples_head = triples_head;
        global_triples_list = triples_list;
        global_cnt = cnt;
...
//        printf("refresh triples\n");
    ...
    if(load){
        ...
        change_triples_head = new long long[N];
        change_triples_list = new long long[change_cnt*3][2];
...
//        printf("cnt %d %d\n", cnt, change_cnt);
    ...
    for(int gid2=0;gid2<N;gid2++){
        int bias = gid2*N;
        for(int id1=0;id1<N;id1++) mini = std::min(mini, cost_matrix_a[bias+id1]);
    }
...
std::vector<std::vector<double>> Optimizer::getCostMatrixForT(
const std::vector<int> &_grid_asses,
const std::vector<int> &_cluster_labels) {
...
std::vector<double> Optimizer::checkCostForT(
...
//    printf("global and now %d %d %d %d\n", global_N, global_hash, N, hash);
    if((global_N==N)&&(global_hash==hash)){
        triples = global_triples;
        triples_head = global_triples_head;
        triples_list = global_triples_list;
        cnt = global_cnt;
...
//        printf("refresh triples\n");
    ...
    for(int i=0;i<num;i++){
        if((consider!=nullptr)&&(consider[element_asses[i]]==0))continue;
