...
class GridLayout(object):
    ...
    def grid(self, labels, X_feature, true_id, hierarchy, top_labels, filter_labels=None, info_before=None, P=None, scale=1/2, confs_hierarchy=None, shres=0.8):
        ...
        original_filter = filter_labels
        filter_labels = copy.deepcopy(filter_labels)
        ...
        label_partition, partition_labels, top_partition = self.get_label_partition_feature(X_feature, labels,
                                                                                            top_labels, filter_labels,
                                                                                            original_top_labels=original_top_labels)
        ...
        if confs_hierarchy is not None:
            ...
            for p_label in conf_label_list:
                conf_label_list[p_label] = np.array(
                    list(map(lambda x: confs_hierarchy['id_map'][x], conf_label_list[p_label])))
        ...
        _, grid_asses = sort_with_las(np.arange(N, dtype='int'), X_feature, radius_factor=0.85)
        # grid_asses, grid_new = AssignQAP(np.arange(N, dtype='int'), np.zeros(N, dtype='int'), feature=X_feature)
        ...
        labels_full = np.concatenate(
            [labels, np.full(N - num, fill_value=label_partition_full.shape[0] - 1, dtype='int')], axis=0)
        ...
        if use_conf and confs_hierarchy is not None:
            ...
            confusion = {"confuse_class": confuse_class, "if_confuse": if_confuse, "conf": now_conf,
                         "conf_vis": conf_vis}
        ...
        if info_before is not None:
            score_dict.update({'stab-shape': IoU_ours, 'stab-position': relative, 'stab-sample': order_score, 'order_ratio': order_score/(order_cnt+1e-12)})
        if confusion is not None:
        ...
        name = "las" + "_" + self.Ctrler.scenario + "_" + self.Ctrler.dataset
