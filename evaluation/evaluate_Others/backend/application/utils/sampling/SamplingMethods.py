...
from application.utils.thread import FuncThread
    ...
    def sample(self, data, category=None):
        ...
        while count < m:
            ...
            while count < m:
                ...
                neighbors = self._neighbors(idx, indexer, selected_indexes, data, category, constraint_matrix, max_pos)
        ...
        selected_indexes = np.array(selected_indexes)
        return np.array(selected_indexes)
    def _neighbors(self, idx, indexer, selected_idx, data, category, constraint_matrix, max_pos):
...
class OutlierBiasedBlueNoiseSampling(SamplingBase):
    ...
    def __str__(self):
        return "OutlierBiasedBlueNoiseSampling"
    ...
    def sample(self, data, category):
        ...
        time_after_get_score = time.time()
        X = np.array(data.tolist(), dtype=np.float64)
        n, d = X.shape
        m = round(n * self.sampling_rate)
        k = int(1 / self.sampling_rate)
        ...
        # X = X.cuda()
        # rewrite the above code to use faiss-GPU
        # res = faiss.StandardGpuResources()
        # print("faiss-GPU resources:", res)
        indexer = faiss.IndexFlatL2(d)
        # indexer = faiss.GpuIndexFlatL2(res, d)
        indexer.add(X)
        time_after_build_index = time.time()
        dist, neighbor = indexer.search(X, k + 1)
        # neighbor, dist = Knn(X, n, d, k + 1, 1, 1, n)
        radius = np.average(np.sqrt(dist[:, -1]))
        ...
        while count < m:
            ...
            for idx in perm:
                ...
                # print(count, fail)
            radius /= 2
        time_after_sampling = time.time()
        print("time for get score:", time_after_get_score - time_start)
        print("time for build index:", time_after_build_index - time_after_get_score)
        print("time for knn:", time_after_knn - time_after_build_index)
        print("time for sampling:", time_after_sampling - time_after_knn)
        print("total time:", time_after_sampling - time_start)
        selected_indexes = np.array(selected_indexes)
        return selected_indexes
...
def RSBS(data, category, canvas_width=1600, canvas_height=900, grid_width=20, threshold=0.02, occupied_space_ratio=0.02, backtracking_depth=4):
    ...
    ffi.cdef(
        ...
        """)
    try:
        dllPath = os.path.join('.', 'application/utils/sampling/RSBS.dll')
...
def Knn(X, N, D, n_neighbors, forest_size, subdivide_variance_size, leaf_number):
    ...
    try:
        t1 = time.time()
        path = os.path.join('.', 'application/utils/sampling/')
        if platform.system() == "Windows":
            sofile = os.path.join(path, 'knnDll.dll')
        else:
            sofile = os.path.join(path, 'libKnn.so')
        C = ffi.dlopen(sofile)
