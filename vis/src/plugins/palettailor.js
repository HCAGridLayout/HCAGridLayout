...
class Palettailor {
  ...
  calculateAlphaShape(extent) {
    let voronoi = d3_voronoi
      ...
      .y(d => d.y)
      .extent(extent);
    // let delaunay = d3.Delaunay.from(extent);
    // console.log(delaunay);
    // return;
    // let voronoi = delaunay.cellPolygons()
    if (!voronoi) {
  ...
  evaluatePalette(palette) {
    ...
    for (let i = 0; i < palette.length; i++) {
      ...
      dis = d3_ciede2000(d3mlab(palette[i]), d3mlab(d3.rgb(this.bg_color)));
      color_discrimination_constraint =
        color_discrimination_constraint > dis
          ? dis
          : color_discrimination_constraint;
